          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                {isNew ? 'Create a new material purchase order line' : `Editing: ${data?.['Material Purchase Order Line'] || ''}`}
              </p>
            </div>
          </div>
          <button
            ...
            {allColumns.map((col) => (
              <div key={col} className="space-y-1">
                <label className="block text-sm font-medium text-gray-700">
                  {col}
                  {REQUIRED_FIELDS.includes(col) && <span className="text-red-500 ml-1">*</span>}
                </label>
                <input
                  type={col.toLowerCase().includes('date') ? 'date' : (col === 'Quantity' ? 'number' : 'text')}
                  value={formData[col] ?? ''}
                  onChange={e => handleInputChange(col, col === 'Quantity' ? Number(e.target.value) : e.target.value)}
                  className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${errors[col] ? 'border-red-300' : 'border-gray-300'}`}
                  placeholder={`Enter ${col.toLowerCase()}`}
                />
                {errors[col] && (
                  <p className="text-sm text-red-600 flex items-center space-x-1">
                    <AlertTriangle className="h-4 w-4" />
                    <span>{errors[col]}</span>
                  </p>
                )}
              </div>
            ))}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[99999999] p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
              ...
              <p className="text-gray-700 mb-6">
                Are you sure you want to delete the line "{formData['Material Purchase Order Line']}"?
