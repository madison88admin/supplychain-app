-- Supply Chain Management System - Complete Database Schema (Fixed Version)
-- This schema safely creates all necessary tables and handles existing tables

-- =====================================================
-- USER MANAGEMENT TABLES
-- =====================================================

-- 1. Users table (main user authentication) - Handle existing table
DO $$ 
BEGIN
...
-- =====================================================
-- SAMPLE DATA INSERTION (Safe - won't duplicate)
-- =====================================================
...
-- Insert sample users with proper roles and departments (only if they don't exist)
...
-- Insert sample products (only if suppliers exist)
INSERT INTO products (product_code, name, description, category, unit_of_measure, standard_cost, selling_price, supplier_id, created_by) 
SELECT 
  'PROD001', 'Cotton T-Shirt', '100% Cotton Basic T-Shirt', 'Apparel', 'Piece', 5.50, 12.99, s.id, u.id
FROM suppliers s, users u 
WHERE s.supplier_code = 'SUP001' AND u.email_address = 'admin@madison88.com'
ON CONFLICT (product_code) DO NOTHING;

INSERT INTO products (product_code, name, description, category, unit_of_measure, standard_cost, selling_price, supplier_id, created_by) 
SELECT 
  'PROD002', 'Denim Jeans', 'Classic Blue Denim Jeans', 'Apparel', 'Piece', 15.75, 29.99, s.id, u.id
FROM suppliers s, users u 
WHERE s.supplier_code = 'SUP002' AND u.email_address = 'admin@madison88.com'
ON CONFLICT (product_code) DO NOTHING;

INSERT INTO products (product_code, name, description, category, unit_of_measure, standard_cost, selling_price, supplier_id, created_by) 
SELECT 
  'PROD003', 'Wool Sweater', 'Premium Wool Winter Sweater', 'Apparel', 'Piece', 25.00, 49.99, s.id, u.id
FROM suppliers s, users u 
WHERE s.supplier_code = 'SUP003' AND u.email_address = 'admin@madison88.com'
ON CONFLICT (product_code) DO NOTHING;
...
DROP POLICY IF EXISTS "Admins can view all data" ON purchase_orders;
CREATE POLICY "Admins can view all data" ON purchase_orders FOR SELECT USING (true);

DROP POLICY IF EXISTS "Admins can modify all data" ON purchase_orders;
